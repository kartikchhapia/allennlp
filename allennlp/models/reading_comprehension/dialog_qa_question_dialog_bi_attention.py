...
class DialogQAQuesDiaBiAttention(Model):
    ...
    def __init__(self, vocab: Vocabulary,
        ...
        self.t = TimeDistributed(torch.nn.Linear(self._encoding_dim * 2, self._encoding_dim))
    ...
    def forward(self,  # type: ignore
        ...
        batch_size, max_dia_count, max_dia_len, _ = dialog['token_characters'].size()
        ...
        embedded_dialog = self._text_field_embedder(dialog, num_wrapping_dims=1)
        embedded_question = embedded_question.reshape(total_qa_count, max_q_len,
                                                      self._text_field_embedder.get_output_dim())
        embedded_dialog = embedded_dialog.reshape(total_qa_count, max_dia_len,
                                                      self._text_field_embedder.get_output_dim())
        ...
        dialog_mask = util.get_text_field_mask(dialog, num_wrapping_dims=1).float()
        ...
        if self._num_context_answers > 0:
            ...
            question_num_ind = util.get_range_vector(max_dia_count, util.get_device_of(embedded_dialog))
            question_num_ind = question_num_ind.unsqueeze(-1).repeat(1, max_dia_len)
            question_num_ind = question_num_ind.unsqueeze(0).repeat(batch_size, 1, 1)
            question_num_ind = question_num_ind.reshape(total_qa_count, max_dia_len)
            question_num_marker_emb = self._question_num_marker(question_num_ind)
            embedded_dialog = torch.cat([embedded_dialog, question_num_marker_emb], dim=-1)
        ...
        encoded_dialog = self._variational_dropout(self._phrase_layer(embedded_dialog, dialog_mask))
        ...
        dialog_question_similarity = self._matrix_attention( encoded_question, encoded_dialog)
        ...
        # Shape: (batch_size * max_qa_count, passage_length, question_length)
        dialog_question_attention = util.masked_softmax(dialog_question_similarity, dialog_mask)
        #logger.info("dialog question attention is %s", dialog_question_attention.shape)
        # Shape: (batch_size * max_qa_count, passage_length, encoding_dim)
        question_dialog_vectors = util.weighted_sum(encoded_dialog, dialog_question_attention)
